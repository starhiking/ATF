...
class HeatmapDatasetOnline(data.Dataset):
    def __init__(self, cfg, heatmap_sigma=1.0, heatmap_size=0, heatmap_offset_sigma=1.0, heatmap_offset_size=0,is_train=True,transforms=None,dataset_exp=1):
        global sometimes
        ...
        if heatmap_sigma and heatmap_size :
            self.heatmap_generator = HeatmapGenerator(heatmap_size, heatmap_sigma)
        else:
            self.heatmap_generator = None

        if heatmap_offset_sigma and heatmap_offset_size:
            #assert self.image_size % heatmap_size==0, "image_size must be a multiple of heatmap_size"
            self.offset_generator = HeatmapGenerator(heatmap_offset_size, heatmap_offset_sigma)
        else:
    ...
    def __getitem__(self,index):
        ...
        if self.heatmap_generator and self.offset_generator:
            """
                return  heatmap  dataset
            """
            fractional_part, integer_part = np.modf(resize_landmark*self.heatmap_size)
            heatmap = self.heatmap_generator.generate_target(integer_part/self.heatmap_size)
            heatmap_offset = self.offset_generator.generate_target(fractional_part)
            return resize_img, resize_landmark, heatmap, heatmap_offset

        return resize_img,resize_landmark.reshape(-1)


def recover_landmark(heatmap,offset):
    landmark=[]
    for h,o in zip(heatmap,offset):
        max_idx = np.argmax(h)
        y, x = max_idx // h.shape[0], max_idx % h.shape[0]
        max_idx2 = np.argmax(o)
        delta_y, delta_x = max_idx2 // o.shape[0], max_idx2 % o.shape[0]
        delta_x/=o.shape[0]
        delta_y/=o.shape[1]
        landmark.append([x+delta_x,y+delta_y])
    landmark = np.array(landmark)
    landmark = landmark / heatmap.shape[1]
    return landmark

if __name__ == "__main__":
    import json
    with open("experiments/COFW.json") as f:
        cfg = json.load(f)
    # os.environ['CUDA_VISIBLE_DEVICES'] = '0'
    # transform = transforms.Compose([transforms.ToTensor(),
    #                                 transforms.Normalize(mean=[0.485,0.456,0.406],std=[0.229,0.224,0.225])])
    wflwDataset = HeatmapDatasetOnline(cfg,heatmap_sigma=4.0, heatmap_size=16,
                                        heatmap_offset_sigma=1.0, heatmap_offset_size=1,
                                        is_train=True, transforms=None, dataset_exp=1)

    dataloader = data.DataLoader(wflwDataset,batch_size=8,shuffle=False,num_workers=8,drop_last=False)
    losses = []
    for i,(imgs,landmarks,heatmaps,offsets) in enumerate(dataloader):
        imgs = imgs.numpy().astype(np.int32)
        landmarks = landmarks.numpy().astype(np.float32).reshape(imgs.shape[0], -1,2)
        heatmaps = heatmaps.numpy().astype(np.float32)
        offsets = offsets.numpy().astype(np.float32)
        img_landmarks = imgs.shape[1] * landmarks

        for i,(img,landmark,heatmap,offset) in enumerate(zip(imgs,img_landmarks,heatmaps,offsets)):
            # plt.imshow(img)
            ...
            # plt.show()
            new_landmark = recover_landmark(heatmap,offset)
            new_landmark = new_landmark*img.shape[1]
            loss = np.sqrt(np.sum((landmark-new_landmark)**2, axis=1))

            if landmark.shape[0] == 98 :
                interocular = np.linalg.norm(landmark[60, ] - landmark[72, ])

            elif landmark.shape[0] == 68:
                interocular = np.linalg.norm(landmark[36, ] - landmark[45, ])
            
            elif landmark.shape[0] == 29:
                interocular = np.linalg.norm(landmark[8, ] - landmark[9, ])

            elif landmark.shape[0] == 19:
                interocular = np.linalg.norm(landmark[6,  ] - landmark[11, ])

            nme = np.mean(loss)/interocular
            losses.append(nme)
            print(nme)
